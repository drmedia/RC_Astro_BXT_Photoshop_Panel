...
2. 우측 상단 톱니바퀴 버튼에서 RC-Astro 실행 환경을 확인합니다.
   ...
   - 우측 상단 `?` 버튼에서 빠른 시작, 처리 대상 자동 판정,
     세부 설정과 오류 해결 안내를 패널 안에서 확인할 수 있습니다.
...
3. 상태 카드에서 자동 결정된 처리 대상과 결과 마스크를 확인합니다.
   ...
   - Photoshop 선택 영역 또는 현재 레이어 마스크가 있으면 보이는 레이어를
     합성해 처리하고 해당 영역을 결과 마스크로 적용합니다.
   - 영역이 없고 현재 레이어가 전체 프레임 픽셀 레이어 또는 스마트
     오브젝트이면 현재 레이어만 처리합니다. 이 방식이 기본입니다.
   - 그 밖의 조정·텍스트·그룹·부분 크기 레이어는 `사용 불가`로 표시되며
     처리하지 않습니다.
   - 선택 영역과 레이어 마스크가 모두 있으면 선택 영역을 우선 사용합니다.
   - 결과 마스크가 적용되면 결과 레이어 이름은 BlurXTerminator - Sky입니다.

4. [세부 설정 보기]를 펼쳐 Strength, 세부 조정과 처리 모드를 설정합니다.

   - `일반 이미지`와 `Lunar / Planetary` 탭으로 처리 모드를 선택합니다.
   - Strength 기본값은 0.30이며 연동 범위는 0~0.7입니다.
   - Strength 연동 ON에서는 Sharpen Stars와 Sharpen Nonstellar가 같이
     움직입니다.
   - 연동 OFF에서는 Strength가 비활성화되고 Stars와 Nonstellar 값을
     각각 설정할 수 있습니다.
   - 슬라이더 오른쪽 숫자 입력란에서 정확한 값을 직접 입력할 수 있습니다.
   - 행성 모드에서는 사용하지 않는 Stars와 Halos가 숨겨지고 Nonstellar와
     PSF Diameter만 표시됩니다.
   - [기본값으로 초기화]로 일반 모드와 기본값을 한 번에 복원할 수 있습니다.
